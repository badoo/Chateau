...
public class SubscribeToUpdatedConversationsTest extends BaseRxTestCase {

    private ConversationRepository mMockRepository;
    private SubscribeToUpdatedConversations mTarget;

    @Before
    public void beforeTest() {
        super.beforeTest();
        mMockRepository = mock(ConversationRepository.class);
        mTarget = new SubscribeToUpdatedConversations(mMockRepository);
    }
    ...
    public void whenMessagesForChatRequested_thenRepoIsQueriedForCorrectChatId() throws Exception {
        ...
        when(mMockRepository.query(eq(new ConversationQuery.SubscribeToConversationUpdatesQuery())))
        ...
        // Assert
        verify(mMockRepository, times(1)).query(eq(new ConversationQuery.SubscribeToConversationUpdatesQuery()));
    }
