...
public class SubscribeToNewMessagesTest extends BaseRxTestCase {

    private MessageRepository mMockRepository;
    private SubscribeToNewMessages mTarget;

    @Before
    public void beforeTest() {
        super.beforeTest();
        mMockRepository = mock(MessageRepository.class);
        mTarget = new SubscribeToNewMessages(mMockRepository);
    }
    ...
    public void whenMessagesForChatRequested_thenRepoIsQueriedForCorrectChatId() throws Exception {
        ...
        when(mMockRepository.query(eq(new SubscribeToNewMessagesForConversation(chatId))))
        ...
        // Assert
        verify(mMockRepository, times(1)).query(eq(new SubscribeToNewMessagesForConversation(chatId)));
    }
